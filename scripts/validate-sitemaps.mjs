...
const IMAGE_SITEMAP_ENTRIES = 11; // all duneImages.sitemap screenshots
...
async function main() {
	...
	const duneAwakeningCheats = path.join(DIST, 'dune-awakening-cheats', 'index.html');
	try {
		const cheatsStat = await stat(duneAwakeningCheats);
		if (cheatsStat.size < 2000) {
			fail(`/dune-awakening-cheats/ built as redirect stub (${cheatsStat.size} bytes)`);
			bump();
		} else {
			ok(`/dune-awakening-cheats/ builds as live pillar page (${cheatsStat.size} bytes)`);
		}
	} catch {
		fail('Missing /dune-awakening-cheats/index.html — pillar page not built');
		bump();
	}
