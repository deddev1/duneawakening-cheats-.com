/**
 * Attach duneawakeningcheats.com to Worker islecheat and create DNS CNAMEs.
...
const DOMAINS = ['duneawakeningcheats.com', 'www.duneawakeningcheats.com'];
...
async function ensureCname(name, content) {
	const fqdn = name.includes('.') ? name : `${name}.duneawakeningcheats.com`;
	const existing = await listDns();
	const hit = existing.find(
		(r) => r.name === fqdn || r.name === name || (name === '@' && r.name === 'duneawakeningcheats.com'),
	);
	if (hit) {
	...
	const recordName = name === 'duneawakeningcheats.com' || name === '@' ? 'duneawakeningcheats.com' : name;
...
async function main() {
	...
	await ensureCname('duneawakeningcheats.com', TARGET);
